...
import { DECORATION_FILTERS } from "~/constants";
...
export const DecorationsPage = () => {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        const decorationServices = (data.services || []).filter(
          (s: Service) => s.category === "decorations"
        );
        setServices(decorationServices);
      } catch (err: any) {
  ...
  // Transform service data to match DecorationCard interface
  const transformedDecorations = services.map((service) => {
  ...
  const filteredDecorations = activeFilter
    ? transformedDecorations.filter((decoration) => {
        return (
          decoration.name.toLowerCase().includes(activeFilter.toLowerCase()) ||
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {loading && (
          <div className="text-center py-12 px-4 lg:px-8">
            ...
            <p className="text-gray-600 font-medium">Loading decorations...</p>
          </div>
        )}
        ...
        {/* Empty State */}
        {!loading && !error && filteredDecorations.length === 0 && (
          <div className="text-center py-12 px-4 lg:px-8">
            <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
              <span className="text-gray-400 text-2xl">🌸</span>
            </div>
            <p className="text-gray-600 font-medium">No decorations found</p>
            <p className="text-sm text-gray-500 mt-1">
              {activeFilter
                ? "Try selecting a different filter"
                : "Check back later for new decorations"}
            </p>
          </div>
        )}
